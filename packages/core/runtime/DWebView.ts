...
export class DWebView {
  ...
  async gateway(strBits: Uint8Array) {
    ...
    this.channelId = strChunkPath.substring(
      strChunkPath.lastIndexOf("/channel/") + 9,
      strChunkPath.lastIndexOf("/chunk")
    );
   ...
   * @param headers
   */
  async resolveNetworkHeaderRequest(
    path: string,
    headers: string,
    status = 200,
    statusText = "success"
  ...
  initAppMetaData(metaData: MetaData) {
    ...
    network.syncCallDenoFunction(callNative.initMetaData, metaData);
  }
  ...
  activity(entry: string) {
    ...
    if (this.entrys.toString().match(RegExp(`${entry}`))) {
      if (isAndroid) {
        ...
        jscore.callJavaScriptWith(callNative.openDWebView, entry);
      }
      return;
    }
